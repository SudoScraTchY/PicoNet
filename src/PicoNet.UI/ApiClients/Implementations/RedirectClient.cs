...
public class RedirectClient : IRedirectClient
{
    ...
    public async Task<ErrorOr<RedirectUrlResult?>> ResolveAsync(string shortCode,string? password = null)
    {
        ...
        var response =
            await _http.GetAsync(
                $"api/redirect/{shortCode}{InjectPassword(password)}");
    ...
    private static string? InjectPassword(string? password)
    {
        return string.IsNullOrEmpty(password) ? null : "?password={password}";
    }
}